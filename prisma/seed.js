...
function sizeForStructure(name) {
  return structureSizes[name] || { width: 6.0, height: 3.0 };
}

function buildAddress(zone, roadType) {
  return `Vía ${roadType}, ${zone}`;
}

function buildLandmark(zone, province) {
...
async function upsertZones(provinceMap) {
  for (const province of provinceZones) {
...
async function upsertSimpleList(model, items, mapFn) {
...
function buildAssets({ zones, structureTypesList, roadTypesList }) {
  return zones.map((zone, index) => {
    ...
    return {
      ...
      status,
      notes: index % 7 === 0 ? "Inventario demo - revisar antes de cotizar" : null,
...
function buildFaces({ assets, positions }) {
  const faces = [];
  ...
  assets.forEach((asset, index) => {
    ...
    for (let i = 0; i < faceCount; i += 1) {
      ...
      faces.push({
        ...
        visibilityNotes:
          i === 0 ? "Alta exposición en vía principal" : "Visibilidad media en retorno",
        status: "ACTIVE",
        restrictions: null,
...
async function upsertFaces(faces) {
...
async function upsertCatalogFaces(faces) {
  for (const [index, face] of faces.entries()) {
    ...
    const primaryImageUrl =
      index % 2 === 0 ? imageUrl("face", `${face.asset.code}-${face.code}`) : null;
...
async function upsertCatalogPriceRules(structureTypesList, zones) {
  ...
  const premiumZones = zones.filter((zone) =>
    ["Brisas del Golf", "Costa Verde", "David", "Boquete", "Avenida Central", "Santiago"].includes(
      zone.name
    )
  );
...
async function upsertProductionSpecs(faces, mountingTypeList) {
  for (const [index, face] of faces.entries()) {
...
async function upsertAssetsImages(assets) {
  for (const asset of assets) {
...
async function upsertFaceImages(faces) {
  for (const face of faces) {
...
async function upsertPermits(assets) {
...
async function upsertMaintenance(assets) {
  for (const [index, asset] of assets.entries()) {
...
async function upsertFaceRestrictions(faces, tags) {
...
async function main() {
  ...
  await upsertSimpleList(prisma.facePosition, facePositions, (name) => ({ name }));
  await upsertSimpleList(prisma.mountingType, mountingTypes, (name) => ({ name }));
